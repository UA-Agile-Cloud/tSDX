...
##        outport = portidtoport[node][output_port_id]
        ...
        if node_id == 1: #First node
            ...
            if inport == 1 and outport == 3:
                ...
                result = 0 
                for channel in range(start_channel*space,(end_channel+1)*space,space):
                    device.chan_port_switching(channel=channel,port=inner_port,table=table,MID=MID)
                    if device.reply[2] == chr(MID) and device.reply[4] == '\x00': #No Error
                        logging.debug('Switching channel %s on sub switch %s:OK' % (channel,table))
                        result = result | 0  
                    else:
                        logging.debug('Switching channel %s on sub switch %s:FAIL' % (channel,table))
                        result = result | 1
            elif inport == 1 and outport == 4:
                logging.debug('Output port is %s' % outport)
                inner_port = 1
                table = 1
                result = 0
