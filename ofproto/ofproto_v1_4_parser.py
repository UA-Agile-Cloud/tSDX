...
OpenFlow 1.4+ Optical extension by Weiyang Mo (wmo@optics.arizona.edu)
"""
...
@_set_msg_type(ofproto.OFPT_SETUP_CONFIG_WSS_REQUEST) #0x74
class OFPTSetupConfigWSSRequest(MsgBase):
    #TODO(Weiyang):Documentation
    #TODO(Weiyang):Error Handling
    def __init__(self, datapath,datapath_id,message_id,ITU_standards,node_id,input_port_id,
                 output_port_id,start_channel,end_channel,experiment1=0,experiment2=0):
        super(OFPTSetupConfigWSSRequest, self).__init__(datapath)
        self.datapath_id = datapath_id
        self.message_id = message_id
        self.ITU_standards = ITU_standards
    ...
    def _serialize_body(self):
        ...
        msg_pack_into(ofproto.OFPT_REQUEST_STR,self.buf,offset,
                      self.datapath_id,self.message_id,
                      self.ITU_standards,self.node_id,self.input_port_id,
                      self.output_port_id,self.start_channel,self.end_channel,
                      self.experiment1,self.experiment2)

@_register_parser
@_set_msg_type(ofproto.OFPT_SETUP_CONFIG_WSS_REPLY) #0x75
class OFPTSetupConfigWSSReply(MsgBase):
    #TODO(Weiyang):Documentation
    #TODO(Weiyang):Error Handling
    def __init__(self,datapath):
        super(OFPTSetupConfigWSSReply, self).__init__(datapath)
        
    @classmethod
    def parser(cls, datapath, version, msg_type, msg_len, xid, buf):
    #TODO(Weiyang):Documentation
    #TODO(Weiyang):Error Handling
        msg = super(OFPTSetupConfigWSSReply, cls).parser(
            datapath, version, msg_type,
            msg_len, xid, buf)
        (msg.datapath_id,msg.message_id,msg.result) = struct.unpack_from(ofproto.OFPT_REPLY_STR,
                                                         msg.buf,ofproto.OFP_HEADER_SIZE)
        return msg


@_set_msg_type(ofproto.OFPT_TEARDOWN_CONFIG_WSS_REQUEST) #0x76
class OFPTTeardownConfigWSSRequest(MsgBase):
    #TODO(Weiyang):Documentation
    #TODO(Weiyang):Error Handling
    def __init__(self, datapath,datapath_id,message_id,ITU_standards,node_id,input_port_id,
                 output_port_id,start_channel,end_channel,experiment1=0,experiment2=0):
        super(OFPTTeardownConfigWSSRequest, self).__init__(datapath)
        self.datapath_id = datapath_id
        self.message_id = message_id
        self.ITU_standards = ITU_standards
        self.node_id = node_id
...
@_register_parser
@_set_msg_type(ofproto.OFPT_TEARDOWN_CONFIG_WSS_REPLY) #0x77
class OFPTTeardownConfigWSSReply(MsgBase):
    #TODO(Weiyang):Documentation
    #TODO(Weiyang):Error Handling
    def __init__(self,datapath):
        super(OFPTTeardownConfigWSSReply, self).__init__(datapath)
        
    @classmethod
    def parser(cls, datapath, version, msg_type, msg_len, xid, buf):
    #TODO(Weiyang):Documentation
    #TODO(Weiyang):Error Handling
        msg = super(OFPTTeardownConfigWSSReply, cls).parser(
            datapath, version, msg_type,
            msg_len, xid, buf)
        (msg.datapath_id,msg.message_id,msg.result) = struct.unpack_from(ofproto.OFPT_REPLY_STR,
                                                         msg.buf,ofproto.OFP_HEADER_SIZE)
...
@_set_msg_type(ofproto.OFPT_GET_OSNR_REQUEST) #0x78
class OFPTGetOSNRRequest(MsgBase):
    #TODO(Weiyang):Documentation
    #TODO(Weiyang):Error Handling
    def __init__(self, datapath,datapath_id,message_id,ITU_standards,node_id,port_id,
                 start_channel,end_channel,experiment1=0,experiment2=0):
        super(OFPTGetOSNRRequest, self).__init__(datapath)
        self.datapath_id = datapath_id
        self.message_id = message_id
        self.ITU_standards = ITU_standards
        self.node_id = node_id
    ...
    def _serialize_body(self):
        offset = ofproto.OFP_HEADER_SIZE
        msg_pack_into(ofproto.OFPT_OSNR_REQUEST_STR,self.buf,offset,
                      self.datapath_id,self.message_id,
                      self.ITU_standards,self.node_id,self.port_id,
                      self.start_channel,self.end_channel,
                      self.experiment1,self.experiment2)
        

@_register_parser
@_set_msg_type(ofproto.OFPT_GET_OSNR_REPLY) #0x79
class OFPTGetOSNRReply(MsgBase):
    #TODO(Weiyang):Documentation
    #TODO(Weiyang):Error Handling
    def __init__(self,datapath):
        super(OFPTGetOSNRReply, self).__init__(datapath)
