...
#TODO(Weiyang): Make this module as TD or update received information to TD in another module
topo_db = {}



# The state transition for elctrical switches: HANDSHAKE -> CONFIG -> MAIN
#
...
class OFPHandler(ryu.base.app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        #I prefer to move onto the main mode when receiving features_reply
        #Multi_part is not essential for me..
...
#Extension part start

        
    @set_ev_handler(ofp_event.EventOFPTCHello, HANDSHAKE_DISPATCHER)
    def chello_handler(self, ev):
        """chello handler

        When receiving chello from cdpi, send a cfeature_request
        Set switch state into CONFIG_DISPATCHER
        """
        self.logger.debug('chello ev %s',ev)
        msg = ev.msg
        datapath = msg.datapath
        #Now that only optical switch (CDPI) will send CHELLO, let's
        #simply reply a CFeatures Request and se to config mode
        datapath.set_version(4) #We already know CDPI's switch version 04,simple here
        cfeatures_reqeust = datapath.ofproto_parser.OFPTCFeaturesRequest(datapath)
        datapath.send_msg(cfeatures_reqeust)
        self.logger.debug('move onto config mode')
        datapath.set_state(CONFIG_DISPATCHER)

    @set_ev_handler(ofp_event.EventOFPTCFeatureReply, CONFIG_DISPATCHER)
    def cfeature_reply_handler(self, ev):
        """cfeature_reply handler

        When receiving cfeature_reply from cdpi, send a cget_config_request
        Set switch state into MAIN_DISPATCHER
        """
        msg = ev.msg
        datapath = msg.datapath
        self.logger.debug('switch features ev %s', msg)
        datapath.id = msg.datapath_id
        cget_config_request = datapath.ofproto_parser.OFPTCGetConfigRequest(datapath)
        datapath.send_msg(cget_config_request)
        self.logger.debug('move onto main state')
        datapath.set_state(MAIN_DISPATCHER)

        
    @set_ev_handler(ofp_event.EventOFPTCGetConfigReply,MAIN_DISPATCHER)
    def cget_config_reply_handler(self, ev):
        """cget_config_reply handler

        When receiving cget_config_reply from cdpi:
            (1)send a cset_config_config_request with attributes from get_config reply
            (2)Periodically sends waveplan_request (every 60 seconds)
            (3)Periodically sends topo_request (every 60 seconds)            
        """
        #TODO(Weiyang): Add intelligence into cset_config
        msg = ev.msg
        datapath = msg.datapath
        topology_request = datapath.ofproto_parser.OFPTTopoRequest(datapath)
        waveplan_request = datapath.ofproto_parser.OFPTWaveplanRequest(datapath)
        cset_config = datapath.ofproto_parser.OFPTCSetConfig(datapath,datapath.id,
                                                             msg.element_id,msg.identity_id,
                                                             msg.itu_standards,msg.min_input_power,
                                                             msg.max_output_power)

        #Periodically request topology and waveplan
        def send_topology_request():
            threading.Timer(60.0,send_topology_request).start()
            datapath.send_msg(topology_request)
        def send_waveplan_request():
            threading.Timer(60.0,send_waveplan_request).start()
            datapath.send_msg(waveplan_request)

        send_topology_request()
        send_waveplan_request()
        datapath.send_msg(cset_config)

    @set_ev_handler(ofp_event.EventOFPTCEchoRequest,[CONFIG_DISPATCHER,MAIN_DISPATCHER])
    def cecho_request_handler(self,ev):
        """cecho_request handler

        When receiving cecho_request from CDPI, sends cecho_reply back.
        """
        msg = ev.msg
        datapath = msg.datapath
        cecho_reply = datapath.ofproto_parser.OFPTCEchoReply(datapath)
        datapath.send_msg(cecho_reply)


    @set_ev_handler(ofp_event.EventOFPTWaveplanReply, MAIN_DISPATCHER)
    def waveplan_reply_handler(self,ev):
        """waveplan_reply handler"""
        #TODO(Weiyang): add received waveplan into TD (Traffic Database)
        #TODO(Weiyang): Link-based waveplan_reply
        msg = ev.msg
        datapath = msg.datapath
        print "Get waveplan_reply msg from datapath:%s" % (dpid_lib.dpid_to_str(msg.datapath_id))
        
        

    @set_ev_handler(ofp_event.EventOFPTTopoReply, MAIN_DISPATCHER)
    def topo_reply_handler(self,ev):
        """topology_reply handler

        Received link will be added to global dictionary topo_db
        """
        #TODO(Weiyang): add received topology into TD in another module
        #TODO(Weiyang): Link-based topology_reply
        msg = ev.msg
        datapath1 = dpid_lib.dpid_to_str(msg.datapath1_id)
        datapath2 = dpid_lib.dpid_to_str(msg.datapath2_id)
        if msg.length == 0:
            print 'a link is disconnected:%s.%s->%s.%s' % (datapath1,
                                                           msg.port1_id,
                                                           datapath2,
                                                           msg.port2_id)
            del topo_db[datapath1.strip('0')][datapath2.strip('0')]
        else:
            print 'a link is added:%s.%s->%s.%s with fiber length %sm' % (datapath1,
                                                                          msg.port1_id,
                                                                          datapath2,
                                                                          msg.port2_id,
                                                                          msg.length)
            topo_db.setdefault(datapath1.strip('0'),{})[datapath2.strip('0')] = msg.length
